...
interface CleaningAreaMasterDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export const CleaningAreaMasterDialog: React.FC<CleaningAreaMasterDialogProps> = ({
  isOpen,
  onClose,
}) => {
  const { data: areas, isLoading } = useCleaningAreas();
  const { mutateAsync: createArea, isPending: isCreating } = useCreateCleaningArea();
  const { mutateAsync: updateArea, isPending: isUpdating } = useUpdateCleaningArea();
  const { mutateAsync: deleteArea, isPending: isDeleting } = useDeleteCleaningArea();
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      await updateArea({ id, data: { name: editName.trim() } });
      setEditingId(null);
      setEditName("");
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("この場所・区域を削除してもよろしいですか？")) {
      try {
        await deleteArea(id);
      } catch (error) {
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[100] w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-2xl bg-white p-0 shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
          ...
          <div className="p-6 overflow-y-auto space-y-4">
            <p className="text-sm text-slate-600 mb-4">
              ここで登録した場所・区域は、日常清掃や定期清掃のドロップダウンで選択可能になります。
            </p>
            ...
            {isLoading ? (
              ...
              <ul className="space-y-2">
                {areas?.map((area) => (
                  <li key={area.id} className="flex items-center justify-between rounded-lg border border-slate-200 p-3 bg-white hover:border-slate-300 transition-colors">
                    {editingId === area.id ? (
