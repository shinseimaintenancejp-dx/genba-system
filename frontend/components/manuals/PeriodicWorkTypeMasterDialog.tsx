...
import {
  ...
  useUpdatePeriodicWorkType,
  useDeletePeriodicWorkType,
} from "@/hooks/useManuals";

interface PeriodicWorkTypeMasterDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export const PeriodicWorkTypeMasterDialog: React.FC<PeriodicWorkTypeMasterDialogProps> = ({
  isOpen,
  onClose,
}) => {
  const { data: workTypes, isLoading } = usePeriodicWorkTypes();
  const { mutateAsync: createWorkType, isPending: isCreating } = useCreatePeriodicWorkType();
  const { mutateAsync: updateWorkType, isPending: isUpdating } = useUpdatePeriodicWorkType();
  const { mutateAsync: deleteWorkType, isPending: isDeleting } = useDeletePeriodicWorkType();
  ...
  const handleCreate = async () => {
    ...
    try {
      await createWorkType({ name: newName.trim() });
      setNewName("");
      setIsAdding(false);
    } catch (error) {
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      await updateWorkType({ id, data: { name: editName.trim() } });
      setEditingId(null);
      setEditName("");
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("この作業内容を削除してもよろしいですか？")) {
      try {
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[100] w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-2xl bg-white p-0 shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
          ...
          <div className="p-6 overflow-y-auto space-y-4">
            <p className="text-sm text-slate-600 mb-4">
              ここで登録した作業内容は、定期清掃の「作業内容」ドロップダウンで選択可能になります。
            </p>
            ...
            {isLoading ? (
              ...
              <ul className="space-y-2">
                {workTypes?.map((wt) => (
                  <li key={wt.id} className="flex items-center justify-between rounded-lg border border-slate-200 p-3 bg-white hover:border-slate-300 transition-colors">
                    {editingId === wt.id ? (
                      ...
                      <>
                        <span className="text-sm font-medium text-slate-700">{wt.name}</span>
