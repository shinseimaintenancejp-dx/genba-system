...
interface FrequencyMasterDialogProps {
  isOpen: boolean;
  onClose: () => void;
}

export const FrequencyMasterDialog: React.FC<FrequencyMasterDialogProps> = ({
  isOpen,
  onClose,
}) => {
  const { data: freqs, isLoading } = useFrequencies();
  const { mutateAsync: createFreq, isPending: isCreating } = useCreateFrequency();
  const { mutateAsync: updateFreq, isPending: isUpdating } = useUpdateFrequency();
  const { mutateAsync: deleteFreq, isPending: isDeleting } = useDeleteFrequency();
  ...
  const handleCreate = async () => {
    ...
    try {
      await createFreq({ name: newName.trim() });
      setNewName("");
      setIsAdding(false);
    } catch (error) {
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      await updateFreq({ id, data: { name: editName.trim() } });
      setEditingId(null);
      setEditName("");
    } catch (error) {
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[100] w-full max-w-lg translate-x-[-50%] translate-y-[-50%] rounded-2xl bg-white p-0 shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
          ...
          <div className="p-6 overflow-y-auto space-y-4">
            <p className="text-sm text-slate-600 mb-4">
              ここで登録した頻度は、契約登録や日常清掃のドロップダウンで選択可能になります。
            </p>
            ...
            {isLoading ? (
              ...
              <ul className="space-y-2">
                {freqs?.map((freq) => (
                  <li key={freq.id} className="flex items-center justify-between rounded-lg border border-slate-200 p-3 bg-white hover:border-slate-300 transition-colors">
                    {editingId === freq.id ? (
                      <div className="flex items-center gap-2 flex-1">
                        <input
                          ...
                          autoFocus
                          onKeyDown={(e) => e.key === "Enter" && handleUpdate(freq.id)}
                        />
                        <button
                          onClick={() => handleUpdate(freq.id)}
                      ...
                      <>
                        <span className="text-sm font-medium text-slate-700">{freq.name}</span>
